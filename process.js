const data = require('./meta_data.json')
...
for (const entry of data) {
  ...
  if (type == 'CoursePage') {
    path = buildPath(buildFromTaxTerms(entry.meta.uuid.course.taxonomyTerms.nodes))
    if (entry.meta.uuid.currentRevision)
      title = entry.meta.uuid.currentRevision.title.trim()
  }
  if (type == 'Page') {
    path = buildPath(buildFromTaxTerms([{navigation: entry.meta.uuid.navigation}]))
    if (entry.meta.uuid.currentRevision)
      title = entry.meta.uuid.currentRevision.title.trim()
  }
  ...
  if (title) {
    ...
    for (let i = path.length - 1; i >= 0; i--) {
      ...
      if (cur.length >= 35 && (outpath.length > 0 || i < path.length - 3))
        break
      if ((path[i] + cur).length <= 60 || (outpath.length == 0 && (path[i] + cur).length <= 66)) {
...
require('fs').writeFileSync('quickbar.json', JSON.stringify(quickbar))

function print(val) {
  return `${val.path.join(' > ')}${val.path.length > 0 ? ' > ' : ''}${val.title}${val.isTax ? ' >' : ''}`
}
