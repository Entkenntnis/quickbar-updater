...
const endpoint = fastMode ? 'https://api.serlo-staging.dev/graphql' : 'https://api.serlo.org/graphql'
  ...
  const toFetch = metaData.filter(entry => {
    ...
    if (ids2.has(entry.id)) {
      if (entry.meta && entry.meta.uuid) {
        if (!['Page', 'CoursePage', 'TaxonomyTerm', 'Article'].includes(entry.meta.uuid.__typename)) {
          return false
        }
      }
      return true
...
function save() {
  require('fs').writeFileSync('meta_data.json', JSON.stringify(metaData))
}
